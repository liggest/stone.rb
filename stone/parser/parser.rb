  ...
  class Parser
    ...
    module Rule
      def rule(cls=nil,&block)
        ...
        parser.syntax_cls= _ =self if self.is_a?(Class) && self <= BasicParser
    ...
    attr_accessor :syntax_cls

    def method_missing(name,...)
      if syntax_cls && syntax_cls.respond_to?(name)
        syntax_cls.send(name,...) # send method call to parser_cls
      else
        super
      end
    end
